...
class Program
{
    ...
    private static void CheckRaffles(object? state)
    {
        ...
        foreach (var raffle in completedRaffles)
        {
    ...
    private static async Task HandleAdminCommands(ITelegramBotClient client, Message message)
    {
        ...
        if (message.Text.StartsWith("/create"))
        {
            ...
            if (parts.Length < 2)
            {
                await client.SendTextMessageAsync(chatId, "Использование: /create НазваниеРозыгрыша [image url]");
                return;
            }

            string giveawayName = parts[1];
            string imageUrl = parts.Length > 2 ? parts[2] : null;


            TimeSpan scheduledTime = DateTime.Now.AddMinutes(5).TimeOfDay;

            var raffle = new Raffle { Name = giveawayName, ScheduledTime = scheduledTime, ImageURL = imageUrl };
            raffles.Add(raffle);
            SaveRaffle(raffle);

            await client.SendTextMessageAsync(chatId, $"Розыгрыш '{giveawayName}' создан! Розыгрыш состоится через 5 минут.");
        }
    ...
    {
        if (raffle.ParticipantIds.Any())
        {
            raffle.RaffleTime = DateTime.Now;
            Random rand = new Random();
            int winnerIndex = rand.Next(raffle.ParticipantIds.Count);
            string winnerName = raffle.Participants[winnerIndex];
            long winnerId = raffle.ParticipantIds[winnerIndex];

            string messageText = $"Поздравляем! {winnerName} ({winnerId}) Вы победитель розыгрыша '{raffle.Name}'!";
            try
            {
            ...
            {
                Console.WriteLine($"Ошибка отправки сообщения победителю: {ex.Message}");
            }
            ...
            {
                string resultMessage = $"Результаты розыгрыша '{raffle.Name}'\nПобедитель - {winnerName} ({winnerId})\n\nПолный список участников: {participantList}";
                try
                {
                    await client.SendTextMessageAsync(participantId, resultMessage);
                }
                catch (Exception ex)
                {
        ...
        else
        {
            ...
            Console.WriteLine(noParticipantsMessage);
        }
    }
